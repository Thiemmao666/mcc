...
spec:
  ...
  volumes:
    ...
    - name: config
      configMap:
        name: {{ include "kube-prometheus.alertmanager.configmapName" . }}
...
{{- include "kube-prometheus.imagePullSecrets" . | nindent 2 }}
  ...
  containers:
    ...
    - name: alertmanager
      ...
      {{- end }}
      args:
        - "--config.file=/opt/bitnami/alertmanager/conf/{{ include "kube-prometheus.alertmanager.configmapKey" . }}"
        ...
        {{- $fullName := include "kube-prometheus.alertmanager.fullname" . }}
